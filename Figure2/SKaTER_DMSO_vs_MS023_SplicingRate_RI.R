...
dataframes <- list(dmso = dmso, d2m = d2m)
...
df <- df[c(1,6,7,5,12)]
colnames(df) <- c('gene','upstreamEE','downstreamES','DMSO_rate','MS023_rate')
...
#d2m <- d2m[d2m$IncLevelDifference < 0,]

df_RI <- merge(df, d2m, by=c("upstreamEE","downstreamES"))
...
ggplot(df, aes(x=variable, fill=variable, y=log10(value)))+
  ...
  scale_color_manual(values=c('#d4d4d4','#7d3593')) +
