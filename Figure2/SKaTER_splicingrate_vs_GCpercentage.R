...
#load in dataframes
d2g <- read.table('SKaTER/Sequencing_Analysis/results/skater_optimize_v0_5_GSK591rep1_database_sorted_skater_optimize_v0_5_GSK591rep1_database_sorted_splicingRates_gc_kde_scatter.txt', header = F, sep = '\t')
d2m <- read.table('SKaTER/Sequencing_Analysis/results/skater_optimize_v0_5_MS023rep1_database_sorted_skater_optimize_v0_5_MS023rep1_database_sorted_splicingRates_gc_kde_scatter.txt', header = F, sep = '\t')
...
#pdf('R_figures/d2m_SKaTER_splicingrate_vs_GCpercentage.pdf', height = 10, width = 10)
ggplot(df, aes(x=group, fill=group, y=rate))+
  ...
  labs(title="", y = "Intron GC Percentage", x="")+
  stat_summary(
...
for (i in 1:nsim) {
  ...
  test <- ks.test(df[df$group == 'RI',]$rate, random$rate, alternative = 'less')
