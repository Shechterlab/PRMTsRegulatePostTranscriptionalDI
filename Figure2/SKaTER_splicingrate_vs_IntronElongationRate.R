...
#load in dataframes
d2g_1 <- read.table('SKaTER/Sequencing_Analysis/results/skater_optimize_v0_5_GSK591rep1_database_sorted_logX=N_logY=N_all_splicingRates_intronTau_kde_scatter.txt', header = F, sep = '\t')
d2g_2 <- read.table('SKaTER/Sequencing_Analysis/results/skater_optimize_v0_5_GSK591rep2_database_sorted_logX=N_logY=N_all_splicingRates_intronTau_kde_scatter.txt', header = F, sep = '\t')
d2m_1 <- read.table('SKaTER/Sequencing_Analysis/results/skater_optimize_v0_5_MS023rep1_database_sorted_logX=N_logY=N_all_splicingRates_intronTau_kde_scatter.txt', header = F, sep = '\t')
d2m_2 <- read.table('SKaTER/Sequencing_Analysis/results/skater_optimize_v0_5_MS023rep2_database_sorted_logX=N_logY=N_all_splicingRates_intronTau_kde_scatter.txt', header = F, sep = '\t')
#dmso <- read.table('SKaTER/Sequencing_Analysis/results/skater_optimize_v0_5_DMSOrep1_database_sorted_skater_optimize_v0_5_DMSOrep1_database_sorted_splicingRates_gc_kde_scatter.txt', header = F, sep = '\t')
d2g <- merge(d2g_1, d2g_2, by = 'V2')
d2g$time <- (d2g$V4.x + d2g$V4.y)/2
d2m <- merge(d2m_1, d2m_2, by = 'V2')
d2m$time <- (d2m$V4.x + d2m$V4.y)/2
...
df <- lapply(df, function(x) {x$distance <- x$downstreamES - x$upstreamEE;x})
df <- lapply(df, function(x) {x$rate <- x$distance/x$time;x})
...
#pdf('R_figures/d2m_SKaTER_splicingrate_vs_intronTau.pdf', height = 10, width = 10)
