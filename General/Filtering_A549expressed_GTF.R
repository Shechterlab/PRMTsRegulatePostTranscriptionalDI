...
hg19_df <- as.data.frame(hg19)
hg19_df$gene_id <- gsub("\\..*","",hg19_df$gene_id)
expressed_hg19 <- hg19_df[hg19_df$gene_id %in% tpm$gene_id,]
#export(expressed_hg19, "gencode.v19.A549expressed.gtf")

#test <-rtracklayer::import('RNAseq/gencode.v19.A549expressed.gtf')


genes <- expressed_hg19[expressed_hg19$type == 'gene',]
genes$score <- '.'
genes <- genes[c(1,2,3,14,8,5)]

genes <- genes[!genes$seqnames == 'chrM',]

genes_plus <- genes[genes$strand == '+',]
genes_minus <- genes[genes$strand == '-',]
#write.table(genes_plus, 'hg19_A549expressed_plus.bed', sep = '\t', quote = F, col.names = F, row.names = F)
