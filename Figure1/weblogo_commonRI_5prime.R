...
upstream_exon <- read.fasta('rMATS_4.0.2/commonRIsequence_upstreamexon.fasta', as.string = TRUE)
...
upstream <- substrRight(upstream_exon, 3)
intron_start <- substr(intron, 1, 6)

upstream_splice <- as.data.frame(cbind(upstream, intron_start))
upstream_splice$sequence <- paste(upstream_splice$upstream, upstream_splice$intron_start, sep = '')

upstream_splice <- data.frame(lapply(upstream_splice, function(v) {
  if (is.character(v)) return(toupper(v))
  else return(v)
}))

seqs <- list(upstream_splice$sequence)
