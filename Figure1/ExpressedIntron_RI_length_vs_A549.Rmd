...
dataframes$intron_length <- NA
for (i in 1:nrow(dataframes)) {
    dataframes[i,50] <-dataframes[i,grep('downstreamES', colnames(dataframes))] - dataframes[i,grep('upstreamEE', colnames(dataframes))]
}
...
stat_box_data <- function(y, upper_limit = max(log10(combined$length)) * 1.15) {
  return( 
    data.frame(
      y = 0.95 * upper_limit,
...
#pdf('../R_figures/d2_PRMTi_RI_length_vs_A549expressed.pdf', height=8, width = 8)
ggplot(combined, aes(x=df, y=log10(length), fill = df)) +
  ...
  geom_hline(yintercept = log10(median(combined[combined$df == 'hg19',]$length)) , linetype = 'dashed')+
  ylim(-3,3.5)+
  stat_summary(
...
for ( i in unique(dfs)) {
  for (j in 1:nsim) {
    ...
    test <- wilcox.test(combined[combined$df == i,]$length, random$length, alternative = 'less')
