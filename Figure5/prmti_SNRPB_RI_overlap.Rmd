...
smbkd_gbm <-  read.table('rMATS_4.0.2/PRJEB10298_GBM/RI.MATS.JunctionCountOnly_without_NA.txt', header = T, sep = '\t')
smbkd_hela <- read.table('rMATS_4.0.2/PRJNA136533_SaltzmanGenesDev/RI.MATS.JCEC.txt', header =T, sep = '\t')

data <- list(d2g_RI = d2g_RI, d2m_RI = d2m_RI, smbkd_gbm = smbkd_gbm, smbkd_hela = smbkd_hela)
...
go.obj_GSK591 <- newGeneOverlap(data$d2g_RI, data$smbkd_hela, genome.size = nrow(expressed.introns))
go.obj_GSK591 <- testGeneOverlap(go.obj_GSK591)
print(go.obj_GSK591)

go.obj_MS023 <- newGeneOverlap(data$d2m_RI, data$smbkd_hela, genome.size = nrow(expressed.introns))
