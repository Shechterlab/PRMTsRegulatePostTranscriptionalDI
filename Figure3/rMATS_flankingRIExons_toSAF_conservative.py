...
with open(rmats, 'r') as f:
    ...
    for line in f:
        ...
        if float(FDR) < float(args.FDR):
            upstreamES = int(upstreamES) + 3
            upstreamEE = int(upstreamEE) - 3
            downstreamES = int(downstreamES) + 3
            downstreamEE = int(downstreamEE) - 3
            outF.write('\t'.join([geneSymbol.strip('"')+ID, chr, str(upstreamES), str(upstreamEE), strand, geneSymbol.strip('"'), GeneID.strip('"')])+'\n')
            outF.write('\t'.join([geneSymbol.strip('"')+ID, chr, str(downstreamES), str(downstreamEE), strand, geneSymbol.strip('"'), GeneID.strip('"')])+'\n')